...
void TLD::init(const Mat& frame1,const Rect& box,FILE* bb_file){
  ...
  iisqsum.create(frame1.rows+1,frame1.cols+1,CV_64F);
	//iisum = new float[(frame1.rows+1)*(frame1.cols+1)];
	//iisqsum = new float[(frame1.rows+1)*(frame1.cols+1)];
  dconf.reserve(100);
  dbb.reserve(100);
  bbox_step =7;
  ...
  classifier.evaluateTh(nXT,nExT);
	...
	//img_buffer = new unsigned char[frame1.rows*frame1.cols];
	//features_buffer = new uchar[scales.size()*10*13*4];
	//col_buffer = new int(frame1.cols);

	myCL.GetPlatformIDs();
	myCL.GetDeviceIDs();
	myCL.CreateContext();
	myCL.CreateCommandQueue();
	myCL.CreateProgram("tld.cl");
	myCL.BuildProgram();
	myCL.CreateKernelFern("detect_fern");
	myCL.CreateKernelMF("MeasureForest");
	myCL.CreateKernelNCC("NCC");
	//myCL.CreateKernelCO("Conf");
...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file){
  ...
  ///Track
  if(lastboxfound && tl){
			//time_start = static_cast<double>(getTickCount());
      track(img1,img2,points1,points2);
			//time_end = static_cast<double>(getTickCount());
			//time_used = (time_end - time_start)/getTickFrequency();
			//printf("track time_used %f ms\n", time_used*1000);
  }
  else{
      tracked = false;
  }
...
void TLD::detect(const cv::Mat& frame){
  ...
  float fern_th = numtrees*classifier.getFernTh();
  Mat patch;

	double Var_buffer[grid.size()];
	int grid_num=0;
	for(int i=0; i<grid.size(); i++)
	{
		Var_buffer[i] = getVar(grid[i],iisum,iisqsum);
		if(Var_buffer[i]>var)
		{
			grid_num++;
		}
	...
	unsigned char img_buffer[img.rows*img.cols];
	int col_buffer = img.cols;
	int grid_buffer[grid_num*3];
	int fern_buffer[grid_num*10];
	float conf_buffer[grid_num];
	...
	for(int i=0; i<img.rows; i++)
	{
		...
		{
			img_buffer[i*img.cols+j] = data[j];
		}
	}
	...
	for(int i=0; i<grid.size(); i++)
	{
		if(Var_buffer[i] > var)
		{
			grid_buffer[iter*3+0] = grid[i].x;
			grid_buffer[iter*3+1] = grid[i].y;
			grid_buffer[iter*3+2] = grid[i].sidx;
			iter++;
		}
	}
	

	myCL.CreateMemBufferFern(img_buffer, img.rows*img.cols, grid_buffer, grid_num*3, classifier.features, scales.size()*10*13*4, &col_buffer);
	myCL.SetKernelArgFern();
	myCL.EnqueueTaskFern(grid_num);
	myCL.ReadBufferFern(fern_buffer, grid_num*10);
	myCL.ReleaseMemObjectFern();

	myCL.CreateMemBufferMF(fern_buffer, grid_num*10, classifier.posteriors, 8192*10, conf_buffer, grid_num);
	myCL.SetKernelArgMF();
	myCL.EnqueueTaskMF(grid_num);
	myCL.ReadBufferMF(conf_buffer, grid_num);
	myCL.ReleaseMemObjectMF();
	...
	for (int i=0;i<grid.size();i++){
		if (Var_buffer[i]>=var)
		{
 			for (int t=0;t<10;t++)
			{
      	tmp.patt[i][t]=fern_buffer[iter*10+t];
  		}

			tmp.conf[i]=conf_buffer[iter];
      if (tmp.conf[i]>fern_th)
			{
      	dt.bb.push_back(i);
      }
			iter++;
  ...
  float nn_th = classifier.getNNTh();
	...
	for (int i=0;i<detections;i++){                                                                                      
	  	patch = frame(grid[dt.bb[i]]);
			getPattern(patch,dt.patch[i],mean,stdev);
  }

	float patch_buffer[detections*225];
	for(int n=0; n<detections; n++)
	{
		for(int i=0; i<15; i++)
		{
			for(int j=0; j<15; j++)
			{
				patch_buffer[n*225+i*15+j] = dt.patch[n].at<float>(i, j);	
			}
		}
	}


	if(classifier.pEx.empty())	
	{
		cout<<"pEx.empty"<<endl;
		if (0>nn_th)
		{
			for(int i=0; i<detections; i++)
			{
				dbb.push_back(grid[dt.bb[i]]);                                         
      	dconf.push_back(1.0);   
			}                                                     
    }
	}
	else if(classifier.nEx.empty())
	{
		cout<<"nEx.empty"<<endl;
		if (1>nn_th)
		{
	...
	{
		float pnEx_buffer[(classifier.pEx.size()+classifier.nEx.size())*225];

		for(int n=0; n<classifier.pEx.size(); n++)
		{
			for(int i=0; i<15;i++)
			{
				//float* pnEx_data = classifier.pEx[i].ptr<float>(j);
				for(int j=0; j<15; j++)
				{
					pnEx_buffer[n*225+i*15+j] = classifier.pEx[n].at<float>(i, j);
				}
			}
		}

		for(int n=0; n<classifier.nEx.size(); n++)
		{
			for(int i=0; i<15;i++)
			{
				//float* pnEx_data = classifier.nEx[i].ptr<float>(j);
				for(int j=0; j<15; j++)
				{
					pnEx_buffer[classifier.pEx.size()*225+n*225+i*15+j] = classifier.nEx[n].at<float>(i, j);
				}
			}
		}
		int pn_size_buffer[2] = {classifier.pEx.size(), classifier.nEx.size()};
		float ncc_buffer[(classifier.pEx.size()+classifier.nEx.size())*detections];
		myCL.CreateMemBufferNCC(patch_buffer, detections*225, 
												  pnEx_buffer, (classifier.pEx.size()+classifier.nEx.size())*225,
										      pn_size_buffer, detections*(classifier.pEx.size()+classifier.nEx.size()));
		myCL.SetKernelArgNCC();
		myCL.EnqueueTaskNCC(detections, classifier.pEx.size()+classifier.nEx.size());
		myCL.ReadBufferNCC(ncc_buffer, detections*(classifier.pEx.size()+classifier.nEx.size()));
		myCL.ReleaseMemObjectNCC();
		...
		for(int n=0; n<detections; n++)
		{
			...
			float maxN = 0;
			float csmaxP = 0;
			int validatedPart = ceil(classifier.pEx.size()*classifier.valid);
			for(int i=0; i<classifier.pEx.size(); i++)
			{
				if(maxP < ncc_buffer[n*(classifier.pEx.size()+classifier.nEx.size())+i])
				{
					maxP = ncc_buffer[n*(classifier.pEx.size()+classifier.nEx.size())+i];
				}
				if(i<validatedPart)
				{	
					csmaxP=maxP;
			...
			for(int i=0; i<classifier.nEx.size(); i++)
			{
				if(maxN < ncc_buffer[n*(classifier.pEx.size()+classifier.nEx.size())+classifier.pEx.size()+i])
				{
					maxN = ncc_buffer[n*(classifier.pEx.size()+classifier.nEx.size())+classifier.pEx.size()+i];
				}
			}
			dt.conf1[n] = (float)(1-maxN)/(2-maxP-maxN);
  		dt.conf2[n] = (float)(1-maxN)/(2-csmaxP-maxN);

			if (dt.conf1[n]>nn_th){                                               
          dbb.push_back(grid[dt.bb[n]]);                                         
          dconf.push_back(dt.conf2[n]);         
      }
		}
...
/// Data generation
  ...
  bad_boxes.clear();
	...
	int* good_boxes_buffer = new int[good_boxes.size()];
	if (good_boxes.size()>num_closest_update){
    //std::nth_element(good_boxes.begin(),good_boxes.begin()+num_closest_update,good_boxes.end(),OComparator(grid));
		for(int i=0; i<good_boxes.size(); i++)
		{
			good_boxes_buffer[i] = good_boxes[i];
			//cout<<grid[good_boxes_buffer[i]].overlap<<endl;
		}
		for(int i=0; i<num_closest_update; i++)
		{	
			for(int j=i+1; j<good_boxes.size(); j++)
			{
				if(grid[good_boxes_buffer[i]].overlap<grid[good_boxes_buffer[j]].overlap)
				{
					int st;	
					st = good_boxes_buffer[i];
					good_boxes_buffer[i] = good_boxes_buffer[j];
					good_boxes_buffer[j] = st;
				}
			}
		}

		good_boxes.resize(num_closest_update);
		for(int i=0; i<good_boxes.size(); i++)
		{
			good_boxes[i] = good_boxes_buffer[i];
		}
  }
	delete[] good_boxes_buffer;
  getBBHull();
 ...
 // bad_boxes.clear();
	...
	//time_used = static_cast<double>(getTickCount());
  //getOverlappingBoxes(lastbox,num_closest_update);
	//time_used = (static_cast<double>(getTickCount()) - time_used)/(getTickFrequency())*1000;
	//cout<<"time used3: "<<time_used<<endl;
