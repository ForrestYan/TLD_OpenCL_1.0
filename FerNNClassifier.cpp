...
void FerNNClassifier::prepare(const vector<Size>& scales){
  ...
  //features = vector<vector<Feature> >(scales.size(),vector<Feature> (totalFeatures));
	features = new uchar[scales.size()*totalFeatures*4];
  RNG& rng = theRNG();
  ...
  for (int i=0;i<totalFeatures;i++){
      ...
      for (int s=0;s<scales.size();s++){
          ...
          //features[s][i] = Feature(x1, y1, x2, y2);
					features[(s*totalFeatures+i)*4+0] = x1;
					features[(s*totalFeatures+i)*4+1] = y1;
					features[(s*totalFeatures+i)*4+2] = x2;
					features[(s*totalFeatures+i)*4+3] = y2;
      }
  ...
  for (int i = 0; i<nstructs; i++) {
	...
	posteriors = new float[nstructs*8192]();
}
...
void FerNNClassifier::getFeatures(const cv::Mat& image,const int& scale_idx, vector<int>& fern){
  ...
  for (int t=0;t<nstructs;t++){
      ...
      for (int f=0; f<structSize; f++){
					x1 = features[scale_idx*130*4+t*10*4+f*4+0];
					y1 = features[scale_idx*130*4+t*10*4+f*4+1];
					x2 = features[scale_idx*130*4+t*10*4+f*4+2];
					y2 = features[scale_idx*130*4+t*10*4+f*4+3];
					res = image.at<uchar>(y1, x1)>image.at<uchar>(y2, x2);
          leaf = (leaf << 1) + res;
      }
      fern[t]=leaf;
  }
...
void FerNNClassifier::update(const vector<int>& fern, int C, int N) {
  ...
  for (int i = 0; i < nstructs; i++) {
      ...
      if (pCounter[i][idx]==0) {
          ...
          posteriors[i*8192+idx] = ((float)(pCounter[i][idx]))/(pCounter[i][idx] + nCounter[i][idx]);
      }
  }
}
